/* ============================================================
   CASHBACK WHEEL — premium prize wheel logic (vanilla JS)
   Public API:  spinWheel("US$ 50.000")  -> always lands exactly there
...
(function () {
  ...
  var PRIZES = [
    { name: 'US$ 5 Cashback',        short: 'US$ 5',    icon: '💵', color: C.green,  weight: 24 },
    { name: 'Extra Spin',            short: 'Extra Spin',icon: '🔄', color: C.blue,   weight: 14 },
    { name: 'US$ 10 Cashback',       short: 'US$ 10',   icon: '💵', color: C.purple, weight: 20 },
    { name: 'Mystery Box',           short: 'Box',    icon: '🎁', color: C.orange, weight: 8  },
    { name: 'US$ 25 Cashback',       short: 'US$ 25',   icon: '💵', color: C.red,    weight: 14 },
    { name: 'iPhone Pro',            short: 'iPhone',   icon: '📱', color: C.gold,   weight: 2  },
    { name: 'US$ 50 Cashback',       short: 'US$ 50',   icon: '💵', color: C.green,  weight: 9  },
    { name: '2 Extra Spins',         short: '2 Spins',  icon: '🔄', color: C.blue,   weight: 7  },
    { name: 'Apple Watch Ultra',     short: 'Watch',    icon: '⌚', color: C.purple, weight: 2  },
    { name: '100% Cashback',         short: '100%',     icon: '💯', color: C.orange, weight: 4  },
    { name: 'Try Again',             short: 'Again',  icon: '❌', color: C.red,    weight: 18 },
    { name: 'MacBook Pro',           short: 'MacBook',  icon: '💻', color: C.gold,   weight: 1  },
    { name: 'US$ 5.000',             short: 'US$ 5K',   icon: '💰', color: C.green,  weight: 3  },
    { name: 'PlayStation 5',         short: 'PS5',      icon: '🎮', color: C.blue,   weight: 2  },
    { name: 'Trip to Hawaii',        short: 'Hawaii',    icon: '🌺', color: C.purple, weight: 2  },
    { name: 'RAM 1500',              short: 'RAM 1500', icon: '🛻', color: C.orange, weight: 1  },
    { name: 'US$ 10.000',            short: 'US$ 10K',  icon: '💰', color: C.red,    weight: 2  },
    { name: 'Tesla Model 3',         short: 'Tesla',    icon: '🚗', color: C.gold,   weight: 1  },
    { name: 'Trip to Dubai',         short: 'Dubai',    icon: '🕌', color: C.green,  weight: 2  },
    { name: 'Trip to Las Vegas',     short: 'Vegas',    icon: '🎰', color: C.blue,   weight: 2  },
    { name: 'US$ 50.000',            short: 'US$ 50K',  icon: '💎', color: C.purple, weight: 1  },
    { name: 'Grand Prize',           short: 'GRAND',   icon: '🏆', color: C.gold,   weight: 1  }
  ];
  ...
  (function buildLegend() {
    var cats = [
      ['Cashback', C.green], ['Spins', C.blue], ['Bonus', C.purple],
      ['Specials', C.orange], ['Cash', C.red], ['Grand Prize', C.gold]
    ];
  ...
  function refreshLock() {
    ...
    unlocked = pr.percent >= 70 || pr.days >= 15;
    if (unlocked) {
      locked.hidden = true;
      ...
      subEl.textContent = 'You reached 70% of your journey. Enjoy your premium spin — every prize below is in play.';
    } else {
      locked.hidden = false;
      shell.classList.add('is-locked');
      shell.classList.remove('ready');
      spinBtn.disabled = true;
      lockSub.textContent = 'You are at ' + pr.percent + '% — reach 70% to spin';
      ...
      subEl.textContent = 'Almost there! You are at ' + pr.percent + '% — reach 70% of your journey to unlock the spin and play for every prize below.';
    }
  }
  ...
  /* ---------- 10. Welcome onboarding free spin ------------- */
  // Reached from the welcome modal via ?welcome=1. This gift spin is an
  // intentional first-time experience: it always lands on US$ 150 Cashback,
  // which is then credited to the balance. One time only.
  (function welcomeInit() {
    if (!/[?&]welcome=1/.test(location.search)) return;
    if (window.LCWallet && window.LCWallet.freeSpinUsed()) return; // already claimed

    // The welcome gift bypasses the 70% gate (it's the onboarding bonus)
    unlocked = true;
    locked.hidden = true;
    shell.classList.remove('is-locked');
    shell.classList.add('ready');
    ...
    // relabel a green cashback wedge to the welcome prize
    var wi = 6;
    PRIZES[wi].name = 'US$ 150 Cashback';
    PRIZES[wi].short = 'US$ 150';
    var segEl = labels.children[wi];
    if (segEl) segEl.innerHTML = '<span class="ico">💵</span><span class="nm">US$ 150</span>';

    isWelcomeSpin = true;
    hubText.textContent = 'FREE SPIN';
    setTimeout(function () { spinToIndex(wi); }, 1000);
  })();
})();
